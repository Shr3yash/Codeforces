...
void solve() {
    ...
    int currentLo = vec[0] - x; 
    int currentHi = vec[0] + x; 

    for(int i = 1; i < n; ++i){
        int subsequentLo = vec[i] - x; 
        int subsequentHi = vec[i] + x; 

        // Overlap check
        if (currentHi < subsequentLo || currentLo > subsequentHi) { 
            // No overlap: change needed
            delta++; 
            currentLo = subsequentLo; 
            currentHi = subsequentHi; 
        } else { 
            // Update to overlapping range
            currentLo = max(currentLo, subsequentLo); 
            currentHi = min(currentHi, subsequentHi); 
        }
    }
