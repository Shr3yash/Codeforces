...
void solve() {
    ...
    // if(ones == s.length() - ones){
    //     cout<<moves<<newline;
    // }else{
        // cout<<static_cast<int>(s.length()) - min(ones, static_cast<int>(s.length()) - ones)<<newline;
        for(moves = 0; moves<s.length(); ++moves){
            if(s[moves] == '0'){
                if(ones>0)ones--;
                else break;
            }else{
