...
void solve() {
    ...
    DEBUG(streak);
        ...
        // number of streaks of Rs is always b + 1
        // because each B separates the streaks
        //printing r for each streak and adding extra r everytime 
        //extra r is smartly added to first 'extra' number of streaks
        // slide all B's except after the last streak
    for (int i = 0; i < b + 1; i++) {
        for (int j = 0; j < streak + (i < extra); j++) {
            cout << 'R';
        }
        if (i < b) {
            cout << 'B';
