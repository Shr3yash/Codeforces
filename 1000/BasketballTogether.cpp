...
void solve() {
    ...
    int teams = 0;
    while (!plyrs.empty()) {
        int maxPower = plyrs.top(); 
        plyrs.pop(); 

        // Calculate the minimum number of players required for the team
        int requiredPlayers = (D / maxPower) + 1;

        if (plyrs.size() >= requiredPlayers - 1) {
            teams++; 
            //Remove the weakest (requiredPlayers - 1) players
            for (int i = 0; i < requiredPlayers - 1; ++i) {
                plyrs.pop();
            }
        } else {
    ...
    cout << teams << '\n';
    /*
    int teams = 0, remainingPlayers = N+1;
    while(remainingPlayers && !plyrs.empty()){
        int currPower = plyrs.top();
        plyrs.pop();
        remainingPlayers = remainingPlayers-1;
        int numOfPlyrs = 1;
        while(currPower*numOfPlyrs <= D && numOfPlyrs < N){
            numOfPlyrs++;
            remainingPlayers--;
        }
        teams++;
