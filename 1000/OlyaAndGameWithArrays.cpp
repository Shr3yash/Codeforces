...
void solve() {
    ...
    int totalArrays, globalMin = INT_MAX;
    cin>>totalArrays;

    vector<int> secondSmallest;
    // map<string, vector<int>> dynamicArrays;

    while(totalArrays--){
        int size;
        cin>>size;
        // string arrayName = "arr" + to_string(totalArrays);
        // vector <int> dynamic(size); 
        // dynamicArrays[arrayName] = dynamic;
        vi arr(size);
        for(int i = 0; i<size; ++i){
            cin>>arr[i];
        }
        int localMin = *min_element(arr.begin(), arr.end());
        globalMin =  min(localMin, globalMin);
        
        //REMOVE LOCAL MIN
        arr.erase(find(arr.begin(), arr.end(), localMin));
        
        //TAKING SECOND SMALLEST IN REGARD
        secondSmallest.push_back(*min_element(arr.begin(), arr.end()));
    }
    ll beautry = globalMin +  accumulate(secondSmallest.begin(), secondSmallest.end(), 0LL) - *min_element(secondSmallest.begin(), secondSmallest.end());
    // (sum of all second smallests and one global min) minus (current second smallest).

    cout<<beautry<<newline;
