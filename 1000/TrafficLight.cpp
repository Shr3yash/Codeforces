...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] == c) {
            // Find the next 'g' after position i
            auto it = upper_bound(greenPositions.begin(), greenPositions.end(), i);
            //upper bound is helper function above used in binary search. 
            //here it helps in finding first position of green, which is 
            //greater than current position.

            //there is a helper func for this in the code which directly returns the index
            //line 107
            int wait;
            if (it != greenPositions.end()) {
                wait = *it - i;
            } else {
                // If no 'g' is found after i, wrap around to the beginning
                wait = n - i + greenPositions[0];
                //n-i to travel + first green
            }
