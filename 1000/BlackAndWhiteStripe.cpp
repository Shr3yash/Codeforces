...
void solve() {
    ...
    for(auto itr: vec){
        ...
        countBlacks =  max(countBlacks, locale);
    }
    if(countBlacks >= k){
        cout<<0<<newline;
        return;
    ...
    int countWhites = 0;
    for(int i = 0; i<k; ++i){
        if(vec[i] == 'W') countWhites++;
        //COUNTING INTIAL FRAME WHITES, V IMP
    }
    //run a sliding window
    int localWhites =  countWhites;
    for(int i = 1; i<=n-k; ++i){
        if(vec[i-1] == 'W') localWhites--;
        //white exits
        if(vec[i+k-1] == 'W') localWhites++;
        //white enters
        countWhites = min(countWhites, localWhites);
    }
    cout<<countWhites<<newline;

/*

// O(N*k) Time Complexity. The one above is O(n).
    ll i = 0, j = i+k,  countWhites = LONG_MAX;
    while(j<=n){
        ll localWhites = 0;
        for(ll x = i; x<j; ++x){
            if(vec[x] == 'W'){
