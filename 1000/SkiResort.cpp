...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (buffer <= maxTemp) {
            localLongestSubarray++;
    // The total number of subarrays of length < minDays 
    // 1, 2, 3, 4, ... mindays-1
    // valid subarrays =  L(L+1)/2 - minDays(minDays-1)/2
        } else {
            if (localLongestSubarray >= minDays) {
                long long L = localLongestSubarray;
                ans += (L - minDays + 1) * (L - minDays + 2) / 2;
            }
            localLongestSubarray = 0;
        }
    }
