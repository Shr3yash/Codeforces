...
void solve() {
    ...
    // goal is to maximise base x height. 
    // for base iterate over all points on side and get the two with most delta
    // for height select a point on the directly opposite side of the two points selected.

    ll w, h;
    cin >> w >> h;

    // Process horizontal sides (y=0 and y=h)
    int k;
    cin >> k;
    vector<ll> x1(k);
    for (int i = 0; i < k; ++i) cin >> x1[i];
    ll max_h0 = x1[k - 1] - x1[0];

    cin >> k;
    vector<ll> x2(k);
    for (int i = 0; i < k; ++i) cin >> x2[i];
    ll max_hh = x2[k - 1] - x2[0];

    // Process vertical sides (x=0 and x=w)
    cin >> k;
    vector<ll> y1(k);
    for (int i = 0; i < k; ++i) cin >> y1[i];
    ll max_v0 = y1[k - 1] - y1[0];

    cin >> k;
    vector<ll> y2(k);
    for (int i = 0; i < k; ++i) cin >> y2[i];
    ll max_vw = y2[k - 1] - y2[0];

    // Calculate maximum area
    ll max_area = max({max_h0 * h, max_hh * h, max_v0 * w, max_vw * w});
    cout << max_area << '\n';
}
