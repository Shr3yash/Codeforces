...
void solve() {
    ...
    // Check for each valid ending
    vector<string> endings = {"00", "25", "50", "75"};

    for (const string& ending : endings) {
        int steps = 0;
        int j = ending.size() - 1;  // Pointer to the ending

        // Traverse from right to left in the number
        for (int i = n - 1; i >= 0; --i) {
            if (num[i] == ending[j]) {
                --j;
                if (j < 0) break; // Found the pair
            } else {
                ++steps;
            }
        }

        if (j < 0) { // Found a valid pair
...
void solve1() {
    ...
    // we have to make last 2 digits 00, 25, 50 or 75
