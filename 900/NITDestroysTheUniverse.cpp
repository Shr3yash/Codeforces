...
void solve1() {
    ...
    for( int i = 0; i<n; ++i){
        ...
        if(buffer != 0){
            ans++;
            while(i<n && buffer != 0){
                i++; //skips the nonzero segment
                if (i < n) {
                    cin >> buffer; // Update buffer to the next element
                }
            }
            i--;
        }

        // if(buffer != 0 && incremented == false){
        //     incremented = true; 
        //     ans++;
        // } else if( buffer == 0 && incremented == true){
        //     incremented =  false;
        // }
...
void solve() {
    ...
    //         operations++;
    //         while (i < n && a[i] != 0) {
    //             ++i; // Skip the entire non-zero segment
    //         }
    //     }
    // }
