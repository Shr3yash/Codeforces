...
void solve() {
    ...
    vector<int> ary(n + 1); // 1-based indexing  problems where queries involve ranges l & r provided in 1-based format.
    //will also help in prefix sum calc
    ...
    while (queries--) {
        ...
        for(int i = l; i<r+1; ++i){
         ...
         For each query, you are iterating over the range 
         [l,r] to compute interimSum. Since the sum of 
         n across all test cases is at most 2×10 
        to pow 5 , this nested loop can result in a time complexity of 
        O(q×n) in the worst case, which exceeds the time limit.*/

        long long rangeSum = prefix[r] - prefix[l - 1];

        long long newTotal = prefix[n] - rangeSum + (long long)(r - l + 1) * k;
