...
void solve() {
    ...
    int pairs =  min(ones, static_cast<int>(s.length() - ones));
    // You need to ensure both arguments to min are of the same type
    // .length() returns size_t (unsigned type)

    
    pairs%2 == 1 ?  cout<<"DA"<<newline:cout<<"NET"<<newline;
