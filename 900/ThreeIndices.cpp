...
void solve() {
    ...
    bool eye = false, jay = false, kay = false;

    for(int i = 0; i<n; ++i){
        cin>>ary[i];
    }

    // pair <pair<int, int>, int> lowestHighestCurrent;
    //stores lowest, highest elem until that index and the current.
    
    /*
    vector<pair <int, int>> lowHigh;
    int minm = INT_MAX, maxm = -1;
    for(int i = 0; i<n; ++i){
        minm = min(ary[i], minm);
        maxm = max(ary[i], maxm);
        lowHigh.push_back({minm, maxm});
    }

    for(int i = 0; i<n; ++i){
        if(lowHigh[i].first < ary[i] && ary[i] > lowHigh[i].second)
    }

    if(eye && jay && kay) cout<<"Yes"<<newline;
    */
    vector<int> prefixMin(n, INT_MAX);
    prefixMin[0] = ary[0];

    for (int i = 1; i < n; ++i) {
        prefixMin[i] = min(prefixMin[i - 1], ary[i]);
    }

/*
   for (int j = 1; j < n - 1; ++j) { // j in middle 
    ...
    */
   for (int j = 1; j < n - 1; ++j) { 
        if (prefixMin[j - 1] < ary[j]) { // Valid i
            for (int k = j + 1; k < n; ++k) { // valid k
                if (ary[k] < ary[j]) {
                    // Found i, j, k
                    cout << "YES\n";
                    // Finding nemo, i mean i, the actual i.
                    for (int i = 0; i < j; ++i) {
                        if (ary[i] == prefixMin[j - 1]) {
                            cout << i + 1 << " " << j + 1 << " " << k + 1 << '\n';
                            return;
                        }
                    }
